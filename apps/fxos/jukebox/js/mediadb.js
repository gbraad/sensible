...
var MediaDB = (function() {
  ...
  function MediaDB(mediaType, metadataParser, options) {
    ...
    openRequest.onupgradeneeded = function(e) {
    	console.log ("openRequest.onupgradeneeded() fires");
    ...
    openRequest.onsuccess = function(e) {
    	console.log ("openRequest.onsuccess() fires");
    ...
    function createObjectStores(db) {
    	console.log ("createObjectStores() in");
      ...
      media.indexes.forEach(function(indexName)  {
    	...
    	console.log ("createObjectStores() out");
    }
    ...
    function initDeviceStorage() {
    	console.log ("initDeviceStorage() in");
      ...
      function getStorageAvailability() {
      	console.log ("getStorageAvailability()");
        var next = 0;
        
        // #if	JASON_HACK
        
        // seems like available() never calls back, for us
        for (var i = 0; i < details.storages.length; i++)
        {
        	var	s = details.storages[i];
        	details.availability [s.storageName] = "available";
        }

				console.log (details.availability);
        ...
        function getNextAvailability() {
					console.log ("getNextAvailability() with next = " + next);
          if (next >= details.storages.length) {
						console.log ("done getting avail, going to setupHandlers()");
          ...
          req.onsuccess = function(e) {
						console.log ("available() success for " + name);
            details.availability[name] = req.result;
            getNextAvailability();
          };
          req.onerror = function(e) {
						console.log ("available() error for " + name);
      ...
      function getState(availability) {
        ...
        if (u === n)
          return MediaDB.NOCARD;
				...
				console.log ("getState() returns READY");
      ...
      function deleteAllFiles(storageName) {
    	...
    	console.log ("initDeviceStorage() out");
    }
  }
  ...
  MediaDB.prototype = {
    ...
    getFileInfo: function getFile(filename, callback, errback) {
      ...
      read.onsuccess = function() {
	    	console.log ("media.db.transaction onsuccess fires");
        if (callback)
    ...
    getFile: function getFile(filename, callback, errback) {
    	console.log ("MediaDB.getFile(" + filename + ")");
      ...
      getRequest.onsuccess = function() {
	    	console.log ("storage.get onsuccess fires");
        callback(getRequest.result);
      };
      getRequest.onerror = function() {
	    	console.log ("storage.get onerror fires");
  ...
  function changeState(media, state) {
  	...
  	console.log ("current state is " + media.state);
